...
async def test_switch_mid_episode(toy_repo, stub_fs, stub_test, stub_llm):
    ...
    trace.add_event(
        "pre_switch", topology="star", epoch=initial_epoch, queued_messages=total_before
    )
    ...
    for agent_id in recipients:
        ...
        if msg:
            trace.add_event("dequeue_during_switch", agent=agent_id, msg_epoch=msg.topo_epoch)

    # Wait for switch to complete
    switch_result = await switch_task

    if switch_result["ok"]:
        trace.add_event("switch_commit", new_topology="chain", new_epoch=switch_result["epoch"])
    else:
        trace.add_event("switch_abort", topology="star", epoch=switch_result["epoch"])
