...
def test_cp_large_n_edge_cases():
    ...
    assert abs(cp_large_zero - expected_zero) < 1e-5, \
        f"CP(0/1000) = {cp_large_zero:.5f}, expected {expected_zero:.5f}"
    assert abs(cp_large_zero - 0.00299) < 1e-4, \
    ...
    assert cp_large_one > cp_large_zero, \
        f"CP(1/1000) = {cp_large_one:.5f} should be > CP(0/1000) = {cp_large_zero:.5f}"
    assert cp_large_one < 0.01, \
        f"CP(1/1000) should be < 1%, got {cp_large_one:.5f}"
    # Expected value approximately 0.00432 (using our beta_inv approximation)
    assert abs(cp_large_one - 0.00432) < 1e-3, \
    ...
    assert cp_large_zero < cp_large_one < cp_2_1000 < cp_5_1000 < cp_10_1000, \
        "CP bounds should increase monotonically with violations"
    
    # Check that 10/1000 (1% empirical) has upper bound < 2%
    assert cp_10_1000 < 0.02, \
        f"CP(10/1000) should be < 2%, got {cp_10_1000:.5f}"