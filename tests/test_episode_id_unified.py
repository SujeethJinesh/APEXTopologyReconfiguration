...
async def test_unified_episode_id_all_topologies(toy_repo, stub_fs, stub_test, stub_llm):
    ...
    for topology in ["star", "chain", "flat"]:
        ...
        assert (
            len(episode_ids) == 1
        ), f"{topology}: All messages should share same episode_id, found: {episode_ids}"

        # Verify the episode_id matches what we created
        assert (
            episode_id in episode_ids
        ), f"{topology}: Episode ID should match created ID {episode_id}"
        ...
        assert (
            episode_id in agent_episode_ids
        ), f"{topology}: Agents should have the created episode_id"
...
async def test_episode_id_failure_injection(toy_repo, stub_fs, stub_test, stub_llm):
    ...
    await runner.run("star", steps=10)
    ...
    assert (
        len(episode_ids) > 1
    ), f"Failure injection should create multiple episode_ids, but found only: {episode_ids}"

    num_ids = len(episode_ids)
    print(f"✅ Failure injection confirmed: Found {num_ids} different episode_ids as expected")
