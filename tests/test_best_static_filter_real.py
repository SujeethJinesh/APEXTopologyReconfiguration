    ...
    with open(best_path, "r") as f:
        for line in f:
            ...
            if "notes" in obj and obj["notes"]:
                static_policies = ["static_star", "static_chain", "static_flat"]
                assert any(
                    p in obj["notes"] for p in static_policies
...
def test_best_static_selection_stats():
    ...
    assert sum(policy_counts.values()) == 300, "Sum of selections should equal total tasks"
    ...
    assert policies_used >= 2, f"Only {policies_used} policy used - expected mix of policies"
