...
async def test_router_overwrites_epoch_at_ingress():
    ...
    assert dequeued.topo_epoch == 1, f"Router should overwrite epoch to 1, got {dequeued.topo_epoch}"
    
    print(f"✅ Router correctly overwrote stale epoch 99 to active epoch 1")
...
async def test_router_epoch_during_switch():
    ...
    assert dequeued1.topo_epoch == 1, f"Pre-switch message should have epoch 1, got {dequeued1.topo_epoch}"
    ...
    assert dequeued3.topo_epoch == 2, f"Post-switch message should have epoch 2, got {dequeued3.topo_epoch}"
    assert dequeued3.payload["phase"] == "during_prepare"
    
    print("✅ Router correctly enforces epoch stamping during switch:")
    print(f"  - Pre-switch: overwrote 999 → 1")
    print(f"  - During PREPARE: overwrote 888 → 2")
    print(f"  - No epoch 2 leakage before COMMIT")
...
async def test_router_epoch_abort_scenario():
    ...
    # Abort the switch
    drops = await router.abort_switch()
    
    # After abort, all messages should be in active queue
    # First message (was already in active with epoch 1)
    dequeued1 = await router.dequeue(AgentID("coder"))
    assert dequeued1 is not None
    assert dequeued1.topo_epoch == 1, f"First message should keep epoch 1, got {dequeued1.topo_epoch}"
    ...
    assert dequeued2.topo_epoch == 2, f"Re-queued message keeps its stamped epoch 2, got {dequeued2.topo_epoch}"
    assert dequeued2.payload["seq"] == 2
    
    print("✅ Router epoch handling during abort:")
    print(f"  - Original in active: kept epoch 1")
    print(f"  - Re-queued from next: kept stamped epoch 2")